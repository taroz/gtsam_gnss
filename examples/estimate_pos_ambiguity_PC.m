%% estimate_pos_ambiguity_PC.m
% Estimate position (x) and ambiguity (b) using DD Pseudorange (P) and DD carrier phase (C)
% Used factors: PseudorangeFactor_X, CarrierPhaseFactor_XB
% Author: Taro Suzuki
clear; close all;
...
for i=progress(1:n)
    ...
    for j=1:nsat
        ...
        if ~isnan(obs.L1.resLdd(i,j))
            ...
            graph.add(gtsam_gnss.CarrierPhaseFactor_XB(sym('x',i), sym('b',i), losvecDD, obs.L1.resLdd(i,j), j-1, obs.L1.lam(j), x_ini(i,:)', noise_robust_L));
        end
    end
end
